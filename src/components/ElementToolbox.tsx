...
import { Type, FileText, Link, Square, Image, Code, FileCode, Columns } from 'lucide-react';

interface ElementToolboxProps {
  onAddElement: (type: 'heading' | 'paragraph' | 'link' | 'button' | 'image' | 'csharp' | 'pagecode' | 'row') => void;
  onExport: () => void;
}
...
const ElementToolbox: React.FC<ElementToolboxProps> = ({ onAddElement, onExport }) => {
  const elementTypes = [
    ...
    { type: 'row' as const, label: 'Add Row', icon: Columns },
